...
export default () => {
    const footerNavs = [
        ...
        },
        {
            label: "Contato",
            items: [
                { href: 'mailto:comercial@lsgdigital.com.br', name: 'Email', icon: <Mail className="w-4 h-4 inline mr-1 text-[#ffb23e]" /> },
                {
                    href: 'https://api.whatsapp.com/send/?phone=5519981331191&text&type=phone_number&app_absent=0',
                    name: 'WhatsApp',
                    icon: (
                        <span className="w-4 h-4 inline mr-1 text-[#ffb23e]">
    ...
    return (
        <footer className="text-gray-300 bg-[#131313] px-4 py-5 max-w-screen-xl mx-auto md:px-8">
            <div className="gap-6 justify-between md:flex">
                <div className="flex-1">
                    ...
                    <form onSubmit={(e) => e.preventDefault()}>
                        ...
                        />
                        <button className="mt-2 p-2.5 rounded-md text-black" style={{ backgroundColor: '#ffb23e' }}>Inscrever-se</button>
                    </form>
                </div>
                <div className="flex-1 mt-10 space-y-6 items-center justify-between sm:flex md:space-y-0 md:mt-0">
                    {footerNavs.map((item, idx) => (
                        <ul className="space-y-4" key={idx}>
                            <h4 className="text-gray-200 font-medium text-sm">{item.label}</h4>
                            {item.items.map((el, idx) => (
                                <li key={idx} className="flex items-center">
                                    <a href={el.href} className="hover:underline hover:text-yellow-400 text-xs flex items-center">
                                        {el.icon} 
            ...
            <div className="mt-8 py-6 border-t border-gray-600 items-center justify-between sm:flex">
                ...
                <div className="mt-6 sm:mt-0">
                    <ul className="flex items-center space-x-4">
                        <li>
                            <a href="https://www.instagram.com/lucas.assessoria/" target="_blank" rel="noopener noreferrer">
                                <Instagram className="w-5 h-5 text-[#ffb23e]" />
                            </a>
                        </li>
                        <li>
                            <a href="https://www.facebook.com/" target="_blank" rel="noopener noreferrer">
                                <Facebook className="w-5 h-5 text-[#ffb23e]" />
                            </a>
                        </li>
                    </ul>
                </div>
